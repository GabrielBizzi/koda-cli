...
import { ErrorField, SelectEmpty } from '@ti_torra/web';
	...
	return (
		...
		>
			...
			<StyledSelect
				...
				labelId={domId}
				required={required}
				className={hasError && (selectClass as never as any)}
			...
			>
				...
				{!children && options?.length > 0 ? (
					options.map((option) => {
						const valueOpt = getValue(option);
						...
						if (isMultiple) {
							return (
								<StyledMenuItem
									...
									key={valueOpt}
									value={valueOpt?.toString()}
								>
									<StyledCheckbox
										size="small"
										checked={
											multiValues.indexOf(
												valueOpt?.toString() as unknown as string,
											) > -1
										}
									/>
