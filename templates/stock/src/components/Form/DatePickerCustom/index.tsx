...
interface CustomDatePickerProps extends TDatePickerProps {
	...
	messageValidation?: Array<{ date: Dayjs; mensagem: string }>;
	label?: string;
	width?: string;
	height?: string;
}
...
export function DatePicker({
	...
	const handleChange = (
		...
		if (
			...
			const messageValidationFinded = messageValidation?.find((mv) =>
				mv.date.isSame(newDate, 'date'),
			);

			setSelectedDate(newDate);
			onChange?.(newDate, ctx);

			return enqueueSnackbar(messageValidationFinded?.mensagem, {
				variant: 'warning',
			});
		}
